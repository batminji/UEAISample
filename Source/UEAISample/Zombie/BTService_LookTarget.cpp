...
void UBTService_LookTarget::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	if (Target && Zombie)
	{
		...
		FRotator LookAtRotation = UKismetMathLibrary::FindLookAtRotation(ZombieLocation, TargetLocation);

		FRotator TargetRotation = FMath::RInterpTo(Zombie->GetActorRotation(), LookAtRotation, DeltaSeconds, 5.0f);
